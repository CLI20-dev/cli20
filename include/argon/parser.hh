...
static auto is_option_looking(std::string_view tok) noexcept -> bool {
...
inline auto tokenize(
  ...
  bool post_separator = false;  // true after "--"
  std::size_t i = 0;
  ...
  while (i < args.size()) {
    ...
    // ── command name ──────────────────────────────────────────────────────
    if (!is_option_looking(tok) && command_names.contains(std::string(tok))) {
      push(TokenType::command, tok, i);
      break;  // args[i+1:] are the sub-command's responsibility
    }

    // ── option-looking token ("--opt" or "--opt=val") ─────────────────────
    if (is_option_looking(tok)) {
      ...
      push(TokenType::option, opt_sv, i);
      ...
      // --opt form: advance, then greedily consume value tokens
      ++i;

      if (is_flag) continue;  // flags take no values
      ...
      while (i < args.size()) {
        ...
        if (is_option_looking(next)) {
          const auto next_eq = next.find('=');
          const std::string next_opt{next_eq != std::string_view::npos
                                         ? next.substr(0, next_eq)
                                         : next};
...
struct Parser {
  ...
  auto parse([[maybe_unused]] std::span<const std::string_view> args)
      -> ParseResult<T> {
    ParseResult<T> result;

    auto [spec_map, command_names] = getOptionSpecMap(result.value);
    for (const auto& [key, nargs] : spec_map) {
      // NOLINTNEXTLINE(cppcoreguidelines-pro-bounds-pointer-arithmetic)
      std::cout << "Option: " << key << ", nargs min: " << nargs.min
                << ", nargs max: " << nargs.max << '\n';
    }

    // auto tokenized = tokenize(args, {}, {});

    return result;
  }

 private:
  std::vector<std::string> long_option_prefix = {"--"};
  std::vector<std::string> short_potion_prefix = {"-"};
  std::vector<std::string> end_of_options_separators = {"--"};

  auto getOptionSpecMap(const T& args)
    ...
    auto t = as_tuple(args);

    [&]<class... Args>(std::type_identity<std::tuple<Args...>>) -> void {
      (..., [&]() -> auto {
        using OptT = std::remove_cvref_t<Args>;
        if constexpr (std::derived_from<OptT, OptionTag>) {
          for (const auto& prefix : long_option_prefix) {
            spec_map.emplace(prefix + std::string(OptT::name.view()),
                             OptT::nargs);
          }
          for (const auto& prefix : short_potion_prefix) {
            if (OptT::short_name != '\0') {
              spec_map.emplace(prefix + std::string(1, OptT::short_name),
                               OptT::nargs);
            }
          }
        }
        if constexpr (std::derived_from<OptT, CommandTag>) {
          command_names.emplace(std::string(OptT::commandName()));
        }
      }());
    }(std::type_identity<std::remove_cvref_t<decltype(t)>>{});
...
//       auto&& [... options] = A{};
//       return (... &&
//       (std::derived_from<std::remove_cvref_t<decltype(options)>,
//       ArgumentTag>));
//     }
...
//     // parser templates will reject non-conforming members at the point of
//     use. return std::is_aggregate_v<A>;
//   }();
// };
//
// struct TokenizeResult {
...
//   args. std::optional<std::span<const std::string_view>> command_tail;
// };
//
// // spec_map      : option/flag keys → nargs  (does NOT contain command names)
// // command_names : set of bare command name strings (no "--" prefix)
...
//     }
//
//     // Command name: stop parsing at this level; hand off remainder to
//     sub-parser if (command_names.contains(std::string(arg))) {
...
//             return std::unexpected(ParseError{.code =
//             ErrorCode::duplicate_argument,
//                                               .kind = ErrorKind::parse,
//                                               .position = static_cast<int>(i),
//                                               .subject = key,
//                                               .detail = "option specified
//                                               multiple times"});
//           }
...
//       // To pass a '--'-prefixed string as a positional, use the '--'
//       end-of-options separator. if (arg.size() > 1 && arg[0] == '-') {
//         return std::unexpected(ParseError{.code = ErrorCode::unknown_option,
//                                           .kind = ErrorKind::parse,
//                                           .position = static_cast<int>(i),
...
//                                         .position = static_cast<int>(i),
//                                         .subject = key,
//                                         .detail = "option specified multiple
//                                         times"});
//     }
//
...
//          i + 1 < args.size() && (!nargs_spec.max.has_value() || count <
//          *nargs_spec.max) && args[i + 1] != "--" &&
...
//           .position = static_cast<int>(i),
//           .subject = key,
//           .detail = std::format("option requires at least {} value(s), but
//           only {} provided",
...
//       return false;  // skip: Param<T>-initialized members prevent constexpr
//       T{}
//     } else {
...
//
//     (..., [&] -> auto {
//       if constexpr (args.type == ArgumentType::option || args.type ==
//       ArgumentType::flag) {
...
//     measurement)
//     // and the description.  ANSI codes are applied when printing, not stored
//     here,
...
//         // Use the Command's own description; fall back to a Description
//         member in SubArgs. std::string_view cmd_desc = opts.description(); if
//         (cmd_desc.empty()) {
//           using SubArgs =
//           std::remove_cvref_t<decltype(detail::castBaseIfCommand(opts))>;
//           cmd_desc = detail::getStructDescription<SubArgs>();
//         }
//         commands_section.push_back({std::string(opts.commandName()),
//         cmd_desc}); has_commands = true;
//       }
//     }());
//
...
//     for (const auto& e : options_section) max_left = std::max(max_left,
//     e.left.size()); for (const auto& e : positionals_section) max_left =
//     std::max(max_left, e.left.size()); for (const auto& e : commands_section)
//     max_left = std::max(max_left, e.left.size()); const std::size_t col =
//     max_left + 4;  // 2 indent + 2 gap
//
//     auto append_section = [&](std::string_view header, const
...
//             if (sep_desc.empty()) sep_desc =
//             detail::getStructDescription<SubArgs>(); if (!sep_desc.empty()) {
//               out += "  ";
//               out += sep_desc;
//             }
//           }
...
//   modernize-avoid-c-arrays) auto parse(int argc, char* argv[]) ->
...
//
//     const auto rest = args.size() > 1 ? args.subspan(1) : std::span<const
//     std::string_view>{};
//
...
//           .detail = std::format("{} positional argument(s) provided but only
//           {} expected",
...
//                                         .subject = missing,
//                                         .detail = "required option was not
//                                         provided"});
//     }
//
...
//             std::remove_cvref_t<decltype(detail::castBaseIfCommand(opts))>;
//             Parser<SubArgs> sub_parser;
//             if (auto r = sub_parser.parse(tail,
//             detail::castBaseIfCommand(opts)); !r) {
//               cmd_error = r.error();
//             } else {
//               opts.markProvided();
//             }
...
//         cmd_error.position += static_cast<int>(args.size() -
//         tokenized->command_tail->size()); return std::unexpected(cmd_error);
//       }
//     }
//
//     return {};
...
//   separately). auto makeOptionSpecMap(const Arguments& args) const {
//     auto& [... options] = args;
//
//     std::unordered_map<std::string, detail::Nargs> specMap;
//
//     (..., [&] -> auto {
//       if constexpr (options.type == ArgumentType::option || options.type ==
//       ArgumentType::flag) {
//         specMap.emplace(std::string("--") + options.longOpt(),
//         options.nargs()); if (options.shortOpt() != '\0') {
//           specMap.emplace(std::string("-") + std::string(1,
//           options.shortOpt()), options.nargs());
//         }
//       }
//     }());
//     return specMap;
...
//   }
//
//   // Build a map from option key (e.g. "--foo", "-f") to a callable that
//   parses
//   // the tokenized values into the corresponding field of `args` and marks it
//   as provided.
//   // `args` must remain alive for as long as the returned map is used.
//   auto makeParseMap(Arguments& args) {
//     auto& [... options] = args;
//
//     std::unordered_map<std::string, std::function<std::expected<void,
//     ParseError>(
...
//           parseMap.emplace(std::string("-") + std::string(1,
//           options.shortOpt()), make_fn);
//         }
//       }
...
//       }) {
//         const std::string long_key = std::string("--") +
//         std::string(opts.longOpt()); const char short_ch = opts.shortOpt();
//         const std::string short_key = short_ch != '\0' ? std::string("-") +
//         short_ch : "";
//
...
// template <class Arguments>
// [[nodiscard]]
// auto parse(std::span<const std::string_view> args) -> std::expected<Arguments,
// ParseError> {
