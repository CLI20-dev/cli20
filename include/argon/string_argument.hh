...
struct Arg<std::vector<std::string>, LongOpt, ShortOpt>
  ...
  constexpr explicit Arg(Param<std::vector<std::string>> p)
      : Base(std::move(p)) {}
...
struct PositionalArgument<std::string> : ArgumentTag {
  ...
  explicit PositionalArgument(Param<std::string> p)
    ...
    if (p.validator)
      validator_ = detail::makeValidator<std::string>(std::move(p.validator));
  }
 ...
 private:
  ...
  std::unique_ptr<detail::ValidatorBase<std::string>> validator_;
};
